...
def ingest_fulfilment(spark: SparkSession, *, dt: str, landing_base_path: str, bronze_base_path: str) -> None:
    ctx = new_run_context(dt=dt, landing_base_path=landing_base_path, bronze_base_path=bronze_base_path)

    fe_path = dt_partition_path(ctx.landing_base_path, "fulfilment_events", ctx.dt)
    fe_src = f"{fe_path}/fulfilment_events.jsonl"

    fe_df = _with_ingestion_metadata(
        _read_jsonl(spark, fe_src),
        dt=ctx.dt,
        load_id=ctx.load_id,
    ...
    fe_out = bronze_table_path(ctx.bronze_base_path, "brz_retail_fulfilment_events")
    (
        fe_df.write.format("delta")
        .mode("overwrite")
