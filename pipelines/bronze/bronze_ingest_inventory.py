...
def ingest_inventory(spark: SparkSession, *, dt: str, landing_base_path: str, bronze_base_path: str) -> None:
    ctx = new_run_context(dt=dt, landing_base_path=landing_base_path, bronze_base_path=bronze_base_path)

    inv_path = dt_partition_path(ctx.landing_base_path, "inventory", ctx.dt)
    inv_src = f"{inv_path}/inventory_snapshot.jsonl"

    inv_df = _with_ingestion_metadata(
        _read_jsonl(spark, inv_src),
        dt=ctx.dt,
        load_id=ctx.load_id,
    ...
    inv_out = bronze_table_path(ctx.bronze_base_path, "brz_retail_inventory_snapshot")
    (
